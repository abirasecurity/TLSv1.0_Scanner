...
def check_tls10(ip, port):
    try:
        ...
        if "TLSv1.0   enabled" in output:
            print(f"[!] TLSv1.0 ENABLED on {ip}:{port}")
            return (ip, port, True)
        else:
...
def print_basic_results(all_results):
    ...
    if vulnerable_hosts:
        print("[!] Hosts with TLSv1.0 ENABLED:\n")
        for ip, port in vulnerable_hosts:
            print(f"    - {ip}:{port}")
    else:
        print("[✓] No hosts were found with TLSv1.0 enabled.")
...
def print_remediation_results(all_results):
    ...
    if vulnerable_hosts:
        print("[!] Hosts with TLSv1.0 still ENABLED:\n")
        for ip, port in vulnerable_hosts:
    ...
    print(f"[*] Hosts with TLSv1.0 enabled: {len(vulnerable_hosts)}")
    print(f"[*] Hosts with TLSv1.0 disabled: {len(remediated_hosts)}")
