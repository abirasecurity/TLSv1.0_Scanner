...
def check_tls10(ip, port):
    try:
        # Use the lock to ensure complete line printing
        with print_lock:
        ...
        # Look for the exact line indicating TLSv1.0 is enabled
        if "TLSv1.0   enabled" in output:
            with print_lock:
                print(f"{Fore.YELLOW}[!] TLSv1.0 ENABLED on {ip}:{port}")
            return (ip, port, True)
        else:
...
def print_basic_results(all_results):
    ...
    if vulnerable_hosts:
        print(f"{Fore.YELLOW}{Style.BRIGHT}[!] Hosts with TLSv1.0 ENABLED:\n")
        for ip, port in vulnerable_hosts:
            print(f"{Fore.YELLOW}    - {ip}:{port}")
    else:
        print(f"{Fore.GREEN}{Style.BRIGHT}[✓] No hosts were found with TLSv1.0 enabled.")

    if non_vulnerable_hosts:
        print(f"\n{Fore.GREEN}[+] Hosts with TLSv1.0 DISABLED:\n")
...
def print_remediation_results(all_results):
    ...
    if vulnerable_hosts:
        print(f"{Fore.YELLOW}{Style.BRIGHT}[!] Hosts with TLSv1.0 still ENABLED:\n")
        for ip, port in vulnerable_hosts:
    ...
    print(f"{Fore.YELLOW}[*] Hosts with TLSv1.0 enabled: {len(vulnerable_hosts)}")
    print(f"{Fore.GREEN}[*] Hosts with TLSv1.0 disabled: {len(remediated_hosts)}")

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Detect TLSv1.0 Support using sslscan")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
        futures = [executor.submit(check_tls10, ip, port) for ip, port in targets]
